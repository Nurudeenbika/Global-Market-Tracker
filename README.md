...
└── src/
    ...
    │
    ├── environments/
    │   ├── environment.ts            # Dev config (API URLs, intervals)
    │   └── environment.prod.ts       # Prod config
    │
    ├── styles/
    │   └── main.scss                 # Global design tokens + resets
    │
    └── app/
        ...
        ├── core/
        │   ├── models/
        │   │   ├── asset.model.ts         # Asset, AssetDetail, MarketFilter types
        │   │   ├── api-response.model.ts  # AsyncState, LoadingState, ApiError
        │   │   └── watchlist.model.ts     # WatchlistItem, storage key
        │   │
        │   ├── services/
        │   │   ├── coin-gecko.service.ts   # REST API calls (markets, detail, chart)
        │   │   ├── websocket.service.ts    # Binance WS stream (real-time prices)
        │   │   ├── market-state.service.ts # Signal-based global state + auto-refresh
        │   │   └── watchlist.service.ts    # Watchlist CRUD + localStorage sync
        │   │
        │   └── interceptors/
        │       └── api.interceptor.ts      # Headers, rate-limit retry (429)
        │
        ├── shared/
        │   ├── components/
        │   │   ├── sparkline/
        │   │   │   └── sparkline.component.ts      # Canvas mini-chart
        │   │   ├── skeleton/
        │   │   │   └── skeleton.component.ts       # Animated loading placeholder
        │   │   └── change-badge/
        │   │       └── change-badge.component.ts   # ±% pill with arrow
        │   │
        │   ├── pipes/
        │   │   ├── crypto-currency.pipe.ts  # Smart decimal-place formatter
        │   │   ├── large-number.pipe.ts     # $1.23T / $456.78M abbreviation
        │   │   └── percent-change.pipe.ts   # +3.14% / -0.87% formatting
        │   │
        │   └── directives/
        │       └── price-flash.directive.ts # Green/red flash on price change
        │
        ├── layout/
        │   ├── header/
        │   │   └── header.component.ts     # Top bar: global stats + currency + refresh
        │   └── sidebar/
        │       └── sidebar.component.ts    # Collapsible nav with watchlist badge
        │
        └── features/
            ├── dashboard/
            │   ├── dashboard.component.ts   # Live asset table with filters
            │   ├── dashboard.component.html # Sortable, filterable table template
            │   └── dashboard.component.scss # Table + filter bar styles
            │
            ├── watchlist/
            │   └── watchlist.component.ts   # Saved assets card grid
            │
            └── asset-detail/
                ├── asset-detail.component.ts   # Full detail + Chart.js line chart
                ├── asset-detail.component.html  # Detail layout template
                └── asset-detail.component.scss  # Detail page styles
```
